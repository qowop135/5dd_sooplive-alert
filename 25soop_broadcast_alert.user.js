...
// 기본 체크 간격 300초(5분)로 설정, 내부적으로는 밀리초 단위
...
// 방송 체크 간격 수정 메뉴 (초 단위 입력, 최소 30초 제한)
GM_registerMenuCommand("방송 체크 간격 수정", async () => {
    // alertInterval은 밀리초 단위이므로 초 단위로 변환합니다.
    let currentSeconds = alertInterval / 1000;
    let newInterval = prompt("방송 체크 간격(초)을 입력하세요 (최소 30초):", currentSeconds);
    if (newInterval) {
        ...
        if (newInterval < 30) {
            alert("방송 체크 간격은 최소 30초 이상이어야 합니다.");
            return;
        }
        alertInterval = newInterval * 1000;
        GM.setValue(ALERT_INTERVAL_KEY, alertInterval);
        alert(`방송 체크 간격이 ${newInterval}초로 변경되었습니다.`);
